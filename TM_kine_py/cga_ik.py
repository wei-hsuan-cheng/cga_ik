...
'''
Potential issues: 
1) singular configuration unsolvable
'''
...
def SixDVecToRp():
    ...
    R = Rotation.from_euler('zyx', [180, 45, 180], degrees=True)
    R = R.as_matrix()
    R1 = R[:3, 0].tolist()
    R2 = R[:3, 1].tolist()
    R3 = R[:3, 2].tolist()
...
def TargetPose():
    ...
    # Target pose 
    # # X6 or {6}-org (close to straight pose or zero-configuration) # straight pose is a singular config, thus is unsolvable in our solver (for now, may be fixed in the future)
...
def CGAIK():
    ...
    PPc = (X5 ^ e1 ^ e2 ^ ni) & C5k; # print(PPc); print(PPc.Norm()); 
    PPc = (-1) * PPc.Normalized() # grade-2 point pair
