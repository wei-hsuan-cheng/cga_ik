...
struct MotorIKResult {
    cga::CGA elbowPoint;  // conformal representation of the elbow
...
) 
{
    ...
    CGA elb0 = (CGA(1.0f,0) + T0 * invLenT0) * (T0 | cga::ni);
    ...
    CGA elb1 = (CGA(1.0f,0) + T1 * invLenT1) * (T1 | cga::ni);
    ...
    CGA elb2 = (CGA(1.0f,0) + T2 * invLenT2) * (T2 | cga::ni);

    // End point
    CGA triProduct = (y0 ^ y1 ^ y2 ^ cga::ni);
