...
struct SphericalRobotIKResult {
    float r_b, r_e;
    CGA s0, s1, s2;           // motor (pivot) positions
...
) 
{
    ...
    // Build the rotor R_ee = e^(theta2/2 * e1(3)) * e^(theta1/2 * e2(3)??) * e^(theta0/2 * e12)
    // But remember e3 is now e2, etc. So we do e^(theta2/2 * e1(2)).
    CGA R2 = cga_utils::rot(e12, theta2); // used to be e13, but e3 -> e2, so e13 -> e12
    CGA R1 = cga_utils::rot(e3 * e2, theta1); // used to be e23, but e2->e3 means e23 -> e2(??) 
    CGA R0 = cga_utils::rot(e13, theta0); // was e12. e1^e2 => e1^e3 after swap => e13
    ...
    result.r_b = r_b;
    result.r_e = r_e;
    // We'll store s0..s2 as the 'pivot positions' (meaning r_b*s0 etc.)
    result.s0 = r_b * s0;
    result.s1 = r_b * s1;
    result.s2 = r_b * s2;
...
// // Forward-declare the symbolic “e1, e2, e3, ni, no” from cga namespace
// namespace cga {
//     extern CGA e1, e2, e3, ni, no;
// }
