...
class CGAFK : public rclcpp::Node
{
public:
  CGAFK()
  : Node("cga_fk"), t_(0.0), dh_table_(cga_ik::loadDHTable())
  {
    tf_broadcaster_ = std::make_shared<tf2_ros::TransformBroadcaster>(this);
    timer_ = this->create_wall_timer(16ms, std::bind(&CGAFK::cga_fk_callback_, this));
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<CGAFK>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
